...
class Door(Structure):
    ...
    def interact(self):
        if self.state == "closed":
            # BUG FIX: dùng .deleted thay vì .active
            # Khi player từ chối popup, popup bị delete() nhưng .active vẫn True
            # khiến lần sau không mở được cửa. Kiểm tra .deleted là đáng tin cậy hơn.
            if self.open_prompt and not self.open_prompt.deleted:
                return
            self.open_prompt = None  # dọn tham chiếu cũ trước khi tạo mới
            rect = p.Rect(0, 0, 56, 32)
            rect.center = g.screen_rect.center
            self.open_prompt = controls.Popup(rect, "Open", "Door?", self.confirm_open, set(("main",)), func2=self._cancel_open)

    def _cancel_open(self):
        """Gọi khi player bấm từ chối mở cửa — reset open_prompt đúng cách"""
        if self.open_prompt and not self.open_prompt.deleted:
            self.open_prompt.delete()
        self.open_prompt = None
...
class TextInformation(Structure):
    ...
    def interact(self):
        ...
        bg_surf = gfx.get_surface(self.background_gfx)
        bg_rect = p.Rect(0, 0, bg_surf.get_width(), bg_surf.get_height())
        bg_rect.center = g.screen_rect.center
        controls.TextScreenControl(bg_rect, self.font_name, self.text, set(("main",)), background_gfx=self.background_gfx)
