...
def draw_wrapped_text_on(font_name, string, surface, rect, colour=None, alpha=255, spacing=None):
    """
    Giống draw_wrapped_text nhưng vẽ lên surface tùy chọn thay vì g.screen.
    rect tọa độ tương đối so với surface (không phải g.screen).
    """
    if colour is None:
        colour = g.convert_colour("black")
    words = string.split(" ")
    font = g.fonts[font_name]
    x = rect.x
    y = rect.y
    ...
    for word in words:
        ...
        w, h = font.size(line + " " + word)
        if w > rect.w or "\n" in word:
            if 0 <= y and y + h <= surface.get_height():
                text_surf = font.render(line, False, colour)
                if alpha != 255:
            ...
            line += " " + word
    if line and 0 <= y and y + h <= surface.get_height():
        text_surf = font.render(line, False, colour)
        if alpha != 255:
            text_surf.set_alpha(alpha)
        surface.blit(text_surf, (x, y))
