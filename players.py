...
class Player(creatures.Creature):
    def __init__(self):
        ...
        self._gh_cheat_cooldown = 0  # cooldown tránh kích hoạt liên tục
        self._gt_cheat_cooldown = 0  # cooldown riêng cho cheat minigun
    ...
    def update(self):
        ...
        # CHEAT: Ấn G + H cùng lúc để hồi máu lên 100
        keys = p.key.get_pressed()
        if self._gh_cheat_cooldown > 0:
            self._gh_cheat_cooldown -= g.dt
        if self._gt_cheat_cooldown > 0:
            self._gt_cheat_cooldown -= g.dt

        if keys[p.K_g] and keys[p.K_h] and self._gh_cheat_cooldown <= 0:
            if not self.dead:
                self.health = 100
                self.max_health = max(self.max_health, 100)
                self._gh_cheat_cooldown = 1.0  # 1 giây mới được dùng lại

        # CHEAT: Ấn G + T cùng lúc để nhận Minigun vô hạn đạn
        if keys[p.K_g] and keys[p.K_t] and self._gt_cheat_cooldown <= 0:
            if not self.dead:
                # Kiểm tra đã có Minigun trong túi chưa
                existing = self.inventory.check_for_named_item("Minigun")
                if existing:
                    # Đã có rồi thì chỉ chuyển sang chọn nó
                    for i, slot in enumerate(self.inventory.slots):
                        if slot and slot.name == "Minigun":
                            self.inventory.select_index(i)
                            break
                else:
                    minigun = items.Minigun()
                    added = self.inventory.add_item(minigun)
                    if added:
                        for i, slot in enumerate(self.inventory.slots):
                            if slot and slot.name == "Minigun":
                                self.inventory.select_index(i)
                                break
                self._gt_cheat_cooldown = 1.0
