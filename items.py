...
class Minigun(Gun):
    def __init__(self):
        # Dùng tên "Revolver" để mượn sprite có sẵn, tránh crash thiếu ảnh
        # Bắn cực nhanh (~12 phát/giây), đạn vô hạn
        super().__init__("Revolver", 0.6, 0.08, 250, projectiles=1, spread=0.3, max_ammunition=999999, recharge=0)
        self.name = "Minigun"  # đổi tên thật sau khi init xong
        self.ammunition = self.max_ammunition  # bắt đầu đầy đạn

    def change_ammunition(self, amount):
        # Vô hạn đạn — không bao giờ giảm hay tăng
        pass

    def fire(self):
        # Dùng âm thanh Revolver vì không có Minigun_fire.wav
        self.name = "Revolver"
        super().fire()
        self.name = "Minigun"
